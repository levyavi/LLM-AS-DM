...
2.  Grant a scene-only bonus of +1d6 to the next relevant social check (exactly 1 use).

Scene bonus lifecycle rules:
1.  Scene bonuses are temporary; each use reduces remaining uses by 1, and bonuses are removed when uses reach 0.
2.  All scene-only bonuses are cleared when the scene ends.
